...
sliderInput <- function(inputId, label, min, max, value, step = NULL,
                        ...
                        timezone = NULL, dragRange = TRUE, theme = c(
                          "Shiny", "Flat", "Modern", "Nice", "Simple",
                          "HTML5", "Round", "Square"))
{
  if (!missing(format)) {
  ...
  # recover the theme selected by the user
  theme <- match.arg(
    arg = theme,
    choices = c(
      "Shiny",
  ...
  dep <- list(
    htmlDependency(
      ...
      stylesheet = c(
        ...
        paste0("css/ion.rangeSlider.skin", theme,".css")
      )
    )
  )
